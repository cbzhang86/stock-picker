...
**状态（2026-09-21）**：代码冻结（稳定版 v-G），进入观察期只观察不开发。497 单元测试全过 + 门禁 9/0 + 六维验证（含零让渡无副作用证明）。下个交易日 14:45 起为新版权重首次实盘。
...
**质量保障体系**：497 单元测试（含 09-16 七条硬契约锁、09-19 十六个守卫测试）+ 门禁 9 道 + 硬契约 7 条 + OOS 验证器（2.2 年 × 5225 只）+ 复盘对账（每日，命中率≤50% 熔断）
...
- 单元测试 497 个 / 门禁 9 项 / 硬契约 7 条 / 守卫测试 16 个
